...
- 注册要手机号、要信用卡，甚至可能直接被拒
- 订阅一个月一两百块，只是想先试试值不值这个钱
- 好不容易找到免费的，用着用着额度没了，**不知道什么时候恢复、还剩多少**
- 网上教程张口就是 API key、环境变量、`pip install`，看两行就劝退
- 想学 AI 怎么用，但不想为此先学一整套开发知识

**这个项目要做的事很简单：让一个完全没接触过的人，双击一个文件，几分钟后就能开始和 AI 对话，而且不用为额度提心吊胆。**

## 它好在哪

**① 零门槛 —— 不懂电脑也能装上**

- 双击一个文件就装，不用开终端、不用敲命令、不用先装开发环境
- 不注册任何账号就能先聊起来，想深入用再花 1 分钟去注册
- 全程不要信用卡，不存在哪天忘了取消被自动扣费
- 安装过程只有几个选择题，一路回车就是推荐配置

**② 不用担心额度 —— 这是它最想解决的问题**

- 内置多个免费来源，**一个被限流了自动换下一个**，你不用管切换的事
- 一次回答中途断了，会自动换源并**接着没写完的地方续写**，不用重问一遍
- 真的全部用完时，会明确告诉你「**额度已用完，约 2.5 小时后恢复**」，而不是甩一个看不懂的报错码
- 想要更宽裕，花 1 分钟去 Groq 或智谱注册个免费 key（不要信用卡）接进去，速度和额度立刻上一个台阶
- 每次回答都显示用了多少 token，**心里有数，不会莫名其妙就没了**

**③ 适合边用边学 —— 不只是个聊天框**

- 每次回答后显示耗时和 token 用量，能直观搞懂「token」到底是什么、什么问法更省
- 可以随时换不同的 AI 模型问同一个问题，亲眼看出差别
- 能接进 VS Code、Cursor 这类编辑器，体验「AI 帮你写代码」是怎么回事
- 底层完全开源，想往下深挖随时可以（见文末技术说明）

**④ 用起来不别扭**

- 回答一个字一个字出来，重点加粗、代码变色，不是甩给你一大坨纯文本
- 记得上文，可以连着追问，不用每次重新交代背景
- 本地记着聊过的内容，下次问相关话题会自动带上相关片段，**既省 token 又不用重复解释**

**⑤ 不想要了能删干净**

- 一键卸载，逐项列出每样东西占多大、删了能不能恢复，你自己勾选
- 连它当初为了运行而装的 Node.js 都能一起清掉，不在你电脑里留垃圾
- 删之前会问要不要先备份聊天记录
...
```
  ...
  8  Node.js + 它的依赖（本安装器装的）250M                    ⚠ 其它 Node 项目会失效
  9  npm 下载缓存与日志                缓存 1.0G               可重新下载
```

输入 `a` 就是**一键全删（1–9，含 Node.js）**。

**关于第 8 项**：只有当 Node 确实是本安装器装的（macOS 走 brew、Windows 走 winget）才会自动卸载，脚本会显示它的安装时间供你判断；如果 Node 是你自己从官网装的，脚本不会碰它，只在结束时打印手工卸载步骤。

**Homebrew 本身不在自动删除范围内** —— 它不是这个安装器装的（安装器只是「有就用，没有让你自己装」）。选了第 8 项后，卸载 Homebrew 的官方命令会打印给你，跑不跑由你决定。**Docker 同理**，只删本工具创建的容器与镜像，不动 Docker 本身。
